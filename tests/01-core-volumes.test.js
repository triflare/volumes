...
describe('kxVolumes — block definitions', () => {
  it('declares all required block opcodes', () => {
    ...
    const required = [
      'mountAs', 'formatVolume', 'listVolumes', 'setSizeLimit', 'setFileCountLimit',
      'fileWrite', 'fileRead', 'deletePath', 'listFiles', 'pathCheck', 'joinPaths',
      'setPermission', 'checkPermission', 'exportVolume', 'importVolume',
      'getLastError', 'runIntegrityTest'
    ];
...
describe('kxVolumes — path operations', () => {
  ...
  it('handles empty path segments in join', () => {
    assert.equal(
      extension.joinPaths({ P1: 'vol://', P2: 'file.txt' }),
      'vol://file.txt'
    );
    
    assert.equal(
      extension.joinPaths({ P1: 'vol://dir', P2: '' }),
      'vol://dir'
    );
  });
...
describe('kxVolumes — file listing', () => {
  before(async () => {
    ...
    await extension.fileWrite({ MODE: 'write', STRING: 'd', PATH: 'list_test://subdir/nested/file4.txt' });
  });
...
describe('kxVolumes — export/import', () => {
  before(async () => {
    ...
    await extension.fileWrite({ MODE: 'write', STRING: 'content1', PATH: 'export_test://file1.txt' });
    await extension.fileWrite({ MODE: 'write', STRING: 'content2', PATH: 'export_test://dir/file2.txt' });
  });
...
describe('kxVolumes — integration scenarios', () => {
  ...
  it('handles complex workflow with directories and permissions', async () => {
    ...
    await extension.fileWrite({ MODE: 'write', STRING: 'secret', PATH: vol + 'private/secret.txt' });
  ...
  it('supports round-trip export/import cycle', async () => {
    ...
    const fileList = JSON.parse(await extension.listFiles({ DEPTH: 'all', PATH: 'roundtrip_test://' }));
