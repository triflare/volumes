class tfVolumes {
  ...
  _handleError(e) {
    ...
    else if (message.includes('FORBIDDEN') || e.name === 'ForbiddenError')
      code = 'FORBIDDEN';
  ...
  _pruneEventLog() {
    ...
    for (const watcher of this._watchers.values()) {
      const cursorEventId = this._getWatcherCursorEventId(watcher);
      if (cursorEventId < droppedUntilEventId)
        watcher.cursorEventId = droppedUntilEventId;
    }
  }
      ...
      if (!current.children.has(part)) {
        if (options.createDirs) {
          ...
          if (!this._getPerms(volName, currentRelPath).create) {
            throw new Error(
              'PERMISSION_DENIED: Create permission denied on ' +
                (currentRelPath || 'root')
            );
          }
      ...
      if (vol.type === 'RAM') {
        ...
        if (parent.children.has(name)) {
          ...
          if (existing.type === 'dir')
            throw new Error('TYPE_MISMATCH: Path is a directory');
  ...
  async _getDataURI(args) {
    ...
    try {
      ...
      if (vol.type === 'RAM') {
        ...
        const metaKey = `${volName}${relPath}`;
        const mime =
          this._opfsMeta.get(metaKey) || file.type || 'application/octet-stream';
        this.lastError = JSON.stringify({ status: 'success' });
        ...
        await this._enqueueOPFSVolumeMutation(volName, async () => {
          ...
          for (const key of this._opfsMeta.keys()) {
            if (key === prefix || key.startsWith(prefix + '/'))
              this._opfsMeta.delete(key);
          }
          for (const key of this._opfsPerms.keys()) {
            if (key === prefix || key.startsWith(prefix + '/'))
              this._opfsPerms.delete(key);
          }
  ...
  async exportVolume(args) {
    ...
    try {
      ...
      for (const volName of volsToExport) {
        ...
        if (!this._getPerms(volName, '').control)
          throw new Error(
            `PERMISSION_DENIED: Control permission denied for volume ${volName}`
          );
        const vol = this.volumes[volName];
        ...
        exportObj[volName] = {
          ...
          fileCountLimit:
            vol.fileCountLimit === Infinity ? '__INFINITY__' : vol.fileCountLimit,
          perms: vol.perms,
          tree: tree,
        };
      }
      ...
      for (const volName of volsToImport) {
        ...
        const importedFileCountLimit =
          volData.fileCountLimit === '__INFINITY__' || volData.fileCountLimit === null
            ? Infinity
            : (volData.fileCountLimit ??
              (importedSizeLimit === Infinity ? Infinity : 10000));
  ...
  async setPermission(args) {
    ...
    try {
      ...
      const valueToken =
        typeof args.VALUE === 'string' ? args.VALUE.trim().toLowerCase() : null;
  ...
  async diffSnapshots(args) {
    ...
    try {
      ...
      if (!byVol.has(b))
        throw new Error(`NOT_FOUND: Snapshot ${b} not found for ${volName}`);
  ...
  async runIntegrityTest() {
    ...
    try {
      ...
      try {
        ...
        assert(
          (await this.pathCheck({ PATH: vol + 'fake.txt', CONDITION: 'exists' })) ===
            false,
          'Exists (false)'
        );
        assert(
        ...
        assert(
          (await this.pathCheck({ PATH: vol + 'img.txt', CONDITION: 'exists' })) ===
            false,
          'Delete Verify'
        );
  ...
  async mountArchive(args) {
    ...
    try {
      ...
      if (!data || typeof data !== 'object' || Array.isArray(data)) {
        throw new Error(
          'INVALID_ARGUMENT: JSON must be an object in Volumes export format'
        );
      }
      ...
      if (Object.prototype.hasOwnProperty.call(data, volName)) {
        ...
        if (keys.length === 0)
          throw new Error('INVALID_ARGUMENT: JSON contains no volumes');
        volData = data[keys[0]];
      }
  ...
  async snapshotDelta(args) {
    ...
    try {
      ...
      if (!snap1 || !snap2)
        throw new Error('INVALID_ARGUMENT: Snapshot names are required');
      ...
      for (const [path, entry1] of m1.entries()) {
        if (!m2.has(path))
          deleted.push({ path, mime: entry1.mime, content: entry1.content });
      }
